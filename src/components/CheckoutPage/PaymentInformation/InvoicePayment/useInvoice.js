...
import DEFAULT_OPERATIONS from './invoice.gql';

/**
 * Talon to handle invoice payment.
 *
 ...
 * @param {DocumentNode} props.operations.getInvoiceConfigQuery query to fetch config from backend
 * @param {DocumentNode} props.operations.setPaymentMethodOnCartMutation mutation to set invoice as payment
 *
 * @returns {
 *  payableTo: String,
 ...
 * }
 */
export const useInvoice = props => {
    const operations = mergeOperations(DEFAULT_OPERATIONS, props.operations);

    const {
        getInvoiceConfigQuery,
        setPaymentMethodOnCartMutation
    } = operations;

    const [{ cartId }] = useCartContext();
    const { data } = useQuery(getInvoiceConfigQuery);
    ...
    return {
        payableTo:
            data &&
            data.storeConfig &&
            data.storeConfig.payment_invoice_payable_to,
        mailingAddress:
            data &&
            data.storeConfig &&
            data.storeConfig.payment_invoice_mailing_address,
